...
async function fetchLibrary(domain) {
  const { searchParams } = new URL("https://milo.adobe.com/tools/library");//window.location.href);
...
async function getSuppliedLibrary() {
  const { searchParams } = new URL("https://milo.adobe.com/tools/library");
...
async function combineLibraries(base, supplied) {
  const url = new URL("https://milo.adobe.com/tools/library");
